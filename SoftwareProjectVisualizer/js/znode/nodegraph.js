...
//        if (y > bounds.bottom) y = bounds.bottom;
 ...
 function clear(){
  ...
  var defaultWidth = 100; // originally 100.
  var defaultHeight = 50; // originally 50.
...
//      temp.txt.text(JSON.parse(n.txt));
      temp.txt.text(n.txt);
      if (n.nodename == null) {
        temp.nodename.val("temp name"); // use txt value if nodename is null.
      } else {
  ...
  this.toJSON = function(){
    ...
    for (var i in nodes){
      ...
      json += '"txt" : ' + addSlashes(JSON.stringify(n.txt.text())) + '},';
    }
